...
class DWT_DCT_SVD(BlindWatermark):
    ...
    def _qim_embed(self, val: float, bit: int):
        step = self.qim_step

        if bit == 0:
            target = round(val / step) * step
        else:
            target = round((val - (step / 2)) / step) * step + (step / 2)

        return target

    def _qim_extract(self, val: float):
        step = self.qim_step

        val_1 = round(val / step) * step
        val_2 = round((val - (step / 2)) / step) * step + (step / 2)

        dist_1 = abs(val - val_1)
        dist_2 = abs(val - val_2)
    ...
    def embed(self, image: RGBImage, watermark: RGBImage):
        ...
        for i in range(0, height, self.block_size):
            for j in range(0, width, self.block_size):
                ...
                u, s, vh = np.linalg.svd(dct_block)
                s[0] = self._qim_embed(s[0], watermark_bin[watermark_bit_index])

                s_matrix = np.zeros((self.block_size, self.block_size))
                np.fill_diagonal(s_matrix, s)

                dct_block = u @ s_matrix @ vh

                LL[i : i + self.block_size, j : j + self.block_size] = cv2.idct(dct_block)
                watermark_bit_index += 1

        coeffs_new = (LL, (LH, HL, HH))
        y_channel_watermarked = pywt.idwt2(coeffs_new, self.wavelet)
